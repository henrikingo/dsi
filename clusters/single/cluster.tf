...
resource "aws_instance" "master" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "cd ~; git clone -b evergreen git@github.com:mongodb-labs/YCSB.git",
            ...
            # "mkdir -p ~/hammer; cd ~/hammer; wget --no-check-certificate https://s3-us-west-2.amazonaws.com/dsi-donot-remove/hammer/scripts/bootstrap.sh -O - | bash",
            "echo 'never' | sudo tee /sys/kernel/mm/transparent_hugepage/enabled", 
            "echo 'never' | sudo tee /sys/kernel/mm/transparent_hugepage/defrag", 
            "echo f | sudo tee /sys/class/net/eth0/queues/rx-0/rps_cpus",
            "echo f0 | sudo tee /sys/class/net/eth0/queues/tx-0/xps_cpus",
