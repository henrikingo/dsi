...
def main(args):
    ...
    parser.add_argument(
        '-i', '--ticket', default=None, help='The JIRA ticket associated with this override update')
    parser.add_argument(
        '-p',
        '--project',
        default='performance',
    ...
    parser.add_argument('-t', '--tests', default='.*', help='The test or tests to update')
    ...
    parser.add_argument(
        '-n', '--ndays', action='store_true', help='Override ndays instead of baseline/reference')
    parser.add_argument(
        '--threshold',
        help='New default threshold. Must be used in tandem with'
        '--thread-threshold')
    parser.add_argument(
        '--thread-threshold', help='New thread threshold. Must be used in tandem with --threshold')
    ...
    if use_threshold_overrides:
        update_obj.update_override_threshold(
            float(args.threshold), float(args.thread_threshold), ticket=args.ticket)
    else:
        if args.ndays:
            rule = 'ndays'
        else:
