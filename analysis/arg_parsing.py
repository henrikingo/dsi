    ...
    for arg in args:
        if arg == "log analysis":
            arg_parser.add_argument(
                "--log-analysis",
                help=(
                    "Analyze mongod.log files from the performance test runs for suspect messages. The argument "
                    "to this flag should be a directory that'll be recursively searched for `mongod.log` files; in "
                    "evergreen this is most likely the 'reports/' directory."))

            arg_parser.add_argument(
                "--perf-file",
                help=(
                    "Modifier for --analyze-logs. If you want to only analyze log messages generated during an actual "
                    "test run, and ignore thoes from test setup/transition phases, you can pass in the path to the "
                    "performance results file (probably perf.json) generated by a test runner as an argument to this "
                    "flag, since it contains the necessary test timestamp data."))
