...
"""Unit test for the UpdateOverrideThreshold class. nosetests: run from dsi or dsi/analysis."""
...
from evergreen.update_override_threshold import UpdateOverrideThreshold  # pylint: disable=import-error


class TestUpdateOverrideThreshold(unittest.TestCase):
    """Evaluates how the Override JSON output changes after a call to update_override
    in the UpdateOverrideThreshold class
    """
    ...
    def setUp(self):
        ...
        self.output_file = os.path.join(self.file_path_prefix,
                                        'update_override_thresh_test_output.json')
        self.warn_out = os.path.join(self.file_path_prefix, 'update_override_thresh_test.err')
        self.log_out = os.path.join(self.file_path_prefix, 'update_override_thresh_test.out')

    def test_update_override_threshold(self):  # pylint: disable=too-many-locals
        """Comparison of UpdateOverrideThreshold.update_override output with the expected results
        ...
        update_obj = UpdateOverrideThreshold(project,
                                             git_hash,
                                             variants.split('|'),
                                             task.split('|'),
        ...
        expected_json = os.path.join(self.file_path_prefix,
                                     'update_override_threshold_exp.json.ok')
        expected_warnings = os.path.join(self.file_path_prefix,
                                         'update_override_threshold_exp.err.ok')
        expected_debug = os.path.join(self.file_path_prefix,
                                      'update_override_threshold_exp.out.ok')
