...
def gesd(data, max_outliers=10, significance_level=0.05, mad=False):
    ...
    if max_outliers >= len(data):
        raise ValueError("max_outliers({max_outliers}) <= length({length})".format(
    ...
    if significance_level <= 0.0 or significance_level >= 1.0:
        raise ValueError("invalid significance_level({significance_level})".format(
    ...
    for i in indexes:
        ...
        value = (length - i - 1) * percentage_point / np.sqrt(
            (length - i - 2 + percentage_point**2) * (length - i))
        ...
        LOG.debug(
            "iteration complete",
            z_scores=np.array([
                np.arange(max_outliers, dtype=int), test_statistics, critical_values,
