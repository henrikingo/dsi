...
from dsi.common.command_runner import EXCEPTION_BEHAVIOR
from dsi.common.command_runner import print_trace
from dsi.common.command_runner import run_pre_post_commands
...
class RunTestsTestCase(unittest.TestCase):
    ...
    @patch("dsi.common.host_utils.extract_hosts", return_value=(-1, -1))
    def help_trace_function(self, mock_function, mock_command_dicts, mock_extract_hosts, mock_ssh):
        ...
        with LogCapture(level=logging.ERROR) as log_capture:
            ...
            # yapf: disable
            with patch(
                "dsi.common.host_factory.make_host", return_value=return_value) as mock_make_host:
                # yapf: enable
    ...
    @patch("dsi.common.remote_host.RemoteHost.upload_file")
                ...
                "pre_task": [
                    {
                        "on_workload_client": {
                            "upload_files": [
                                {"source": "workloads.tar.gz", "target": "workloads.tar.gz"}
                            ]
                        }
                    }
                ]
    ...
    @patch("os.path")
    @patch("dsi.common.remote_host.RemoteHost.retrieve_path")
    ...
    def test_print_trace_create_file(self, mock_create_file):
        ...
        mock_command_dicts = [
            {"pre_task": [{"on_workload_client": {"exec_mongo_shell": {"script": "mock script"}}}]},
            {},
        ]
