...
# dsi/tests/unittest-files/terraform/provisioned.initialsync-logkeeper
# dsi/tests/unittest-files/terraform/provisioned.replica
# dsi/tests/unittest-files/terraform/provisioned.shard
...
class TestInfrastructureProvisioning(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {
            ...
            "infrastructure_provisioning": {
                ...
                "evergreen": {"data_dir": FIXTURE_FILES.fixture_dir_path},
                "post_provisioning": [
                    {
                        "on_all_hosts": {
                            "exec": """
        ...
        self.provision_log_path = os.path.join(
            FIXTURE_FILES.fixture_dir_path, ip.PROVISION_LOG_PATH
        )
    ...
    @patch("dsi.infrastructure_provisioning.shutil.copyfile")
    @patch("dsi.infrastructure_provisioning.subprocess.check_call")
    @patch("dsi.infrastructure_provisioning.os.path.isdir")
    @patch("dsi.infrastructure_provisioning.rmtree_when_present")
    def test_check_existing_state(self, mock_rmtree, mock_isdir, mock_check_call, mock_copyfile):
        ...
        with patch("dsi.infrastructure_provisioning.os.path.isfile") as mock_isfile:
        ...
        with patch("dsi.infrastructure_provisioning.os.path.isfile") as mock_isfile:
            with patch("dsi.infrastructure_provisioning.check_version") as mock_check_version:
    ...
    @patch("dsi.infrastructure_provisioning.shutil")
    @patch("dsi.infrastructure_provisioning.subprocess.check_call")
    @patch("dsi.infrastructure_provisioning.os.path.isdir")
    @patch("dsi.infrastructure_provisioning.os.remove")
            ...
            with LogCapture(level=logging.ERROR) as error:
                ...
                error.check(
                    (
                        ...
                        "ERROR",
                        "[error    ] Teardown of existing resources failed. Catching exception and continuing. [dsi.infrastructure_provisioning] \nCommand '['cmd']' returned non-zero exit status 1",  # pylint: disable=line-too-long
                    )
                )
            mock_shutil.rmtree.assert_called_with(evg_data_dir)
            isfile_calls = [
    ...
    @patch("dsi.infrastructure_provisioning.shutil")
    @patch("dsi.infrastructure_provisioning.os.listdir")
    @patch("dsi.infrastructure_provisioning.os.path.isdir")
    def test_setup_evg_dir(self, mock_isdir, mock_listdir, mock_shutil):
        ...
        with patch("dsi.infrastructure_provisioning.os.makedirs") as mock_makedirs:
            mock_isdir.return_value = False
            provisioner = ip.Provisioner(self.config, provisioning_file=self.provision_log_path)
            provisioner.bin_dir = "test/dsi"
            ...
            copy_calls = [
                ...
                call(
                    provisioner.bin_dir + "/infrastructure_teardown.py",
    ...
    @patch("dsi.infrastructure_provisioning.Provisioner.setup_evg_dir")
    @patch("dsi.infrastructure_provisioning.Provisioner.save_terraform_state")
    ...
    @patch("dsi.infrastructure_provisioning.subprocess.check_call")
    @patch("dsi.infrastructure_provisioning.os.remove")
    @patch("dsi.infrastructure_provisioning.os.chdir")
    @patch("dsi.infrastructure_provisioning.os.getcwd")
    @patch("dsi.infrastructure_provisioning.shutil.copyfile")
    def test_save_terraform_state(
        ...
        with patch("dsi.infrastructure_provisioning.glob.glob") as mock_glob:
            mock_open_file = mock_open()
            with patch("dsi.infrastructure_provisioning.open", mock_open_file, create=True):
    ...
    def test_rmtree_when_not_present(self, mock_rmtree, mock_exists, capture):
        ...
        capture.check(
            (
                "dsi.infrastructure_provisioning",
                "INFO",
                u"[info     ] rmtree_when_present: No such path [dsi.infrastructure_provisioning] arg=u''",
            )
        )
    ...
    def test_rmtree_when_present_nopath(self, mock_exists, capture):
        ...
        capture.check(
            (
                "dsi.infrastructure_provisioning",
                "DEBUG",
                u"[debug    ] rmtree_when_present start      [dsi.infrastructure_provisioning] arg=u''",
            ),
            (
                "dsi.infrastructure_provisioning",
                "INFO",
    ...
    def test_print_terraform_errors(self, log_output):
        ...
        log_output.check(
            (
                "dsi.infrastructure_provisioning",
                "INFO",
                u"[info     ] Using terraform binary:        [dsi.infrastructure_provisioning] path=u'test/path/terraform'",
            ),
            (
                "dsi.infrastructure_provisioning",
                "INFO",
                u"[info     ] Redirecting terraform output to file [dsi.infrastructure_provisioning] path=u'{}'".format(
            ...
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ] 2018-03-05T15:45:36.018+0200 [DEBUG] plugin.terraform-provider-aws_v1.6.0_x4: <Response><Errors><Error><Code>InsufficientInstanceCapacity</Code><Message>Insufficient capacity.</Message></Error></Errors><RequestID>bd5b4071-755d-440e-8381-aa09bad52d69</RequestID></Response> [dsi.infrastructure_provisioning] ",
            ),
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ] 2018-03-05T15:45:36.914+0200 [DEBUG] plugin.terraform-provider-aws_v1.6.0_x4: <Response><Errors><Error><Code>RequestLimitExceeded</Code><Message>Request limit exceeded.</Message></Error></Errors><RequestID>6280e71d-9be4-442c-8ddf-00265efeafe6</RequestID></Response> [dsi.infrastructure_provisioning] ",
            ),
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ] 2018-03-05T15:48:36.336+0200 [DEBUG] plugin.terraform-provider-aws_v1.6.0_x4: <Response><Errors><Error><Code>InvalidRouteTableID.NotFound</Code><Message>The routeTable ID 'rtb-509f1528' does not exist</Message></Error></Errors><RequestID>54256eb4-d706-4084-86dc-b7f581006f9f</RequestID></Response> [dsi.infrastructure_provisioning] ",
            ),
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ] 2018-03-05T15:48:47.258+0200 [DEBUG] plugin.terraform-provider-aws_v1.6.0_x4: <Response><Errors><Error><Code>DependencyViolation</Code><Message>Network vpc-a9ed8bd0 has some mapped public address(es). Please unmap those public address(es) before detaching the gateway.</Message></Error></Errors><RequestID>cd102bc6-d598-4bae-80f5-25e62103f9a4</RequestID></Response> [dsi.infrastructure_provisioning] ",
            ),
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ] 2018-03-05T15:49:29.084+0200 [DEBUG] plugin.terraform-provider-aws_v1.6.0_x4: <Response><Errors><Error><Code>InvalidPlacementGroup.Unknown</Code><Message>The Placement Group 'shard-8665ea69-9e76-483a-937b-af68d41d54dd' is unknown.</Message></Error></Errors><RequestID>4264aef8-ae91-40f0-bc16-d914a8dc2cf8</RequestID></Response> [dsi.infrastructure_provisioning] ",
            ),
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ] For more info, see:            [dsi.infrastructure_provisioning] path=u'{}'".format(
            ...
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
                u"[error    ]                                [dsi.infrastructure_provisioning] ",
            ),
            (
                "dsi.infrastructure_provisioning",
                "ERROR",
    ...
    def test_setup_hostnames(self, mock_exec_command, mock_create_file, mock_ssh):
        ...
        with test_config.in_dir(config_files):
            ...
            provisioner = ip.Provisioner(
                real_config_dict, provisioning_file=self.provision_log_path
            )
