    ...
    def test_terraform_call_fails(self, mock_check_output):
        ...
        with LogCapture(level=logging.CRITICAL) as crit:
            ...
            message_1 = (
                ...
                u"http://bit.ly/2ufjQ0R [dsi.bootstrap] "
            )
            message_2 = u"[critical ] Call to terraform failed.      [dsi.bootstrap] "
            crit_expected = {
        ...
        with LogCapture(level=logging.CRITICAL) as crit:
            ...
            crit_expected = {
                (
                    "dsi.bootstrap",
                    "CRITICAL",
                    u"[critical ] See documentation for installing terraform: http://bit.ly/2ufjQ0R [dsi.bootstrap] ",
                ),
                (
                    "dsi.bootstrap",
                    "CRITICAL",
                    u"[critical ] Cannot execute terraform binary file. [dsi.bootstrap] ",
                ),
            }
            self.assertTrue(crit_expected.issubset(crit_logs))
        ...
        with LogCapture(level=logging.CRITICAL) as crit:
            ...
            crit_expected = {
                ...
                (
                    "dsi.bootstrap",
                    "CRITICAL",
                    u"[critical ] No terraform binary file found. [dsi.bootstrap] ",
                ),
            }
            self.assertTrue(crit_expected.issubset(crit_logs))
        ...
        with LogCapture(level=logging.CRITICAL) as crit:
            ...
            crit.check(
                (
                    "dsi.bootstrap",
                    "CRITICAL",
                    u"[critical ] Location specified for bootstrap.yml is invalid. [dsi.bootstrap] ",
                )
            )
