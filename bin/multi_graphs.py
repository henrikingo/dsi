...
Create pyplot graphs from data that was output from multi_analysis.py.
    ...
    def separate_fio_tests(self):
        ...
        canary = {}
        mongodb_tests = {}
        for variant_name, variant_obj in self.agg_results.items():
            for task_name, task_obj in variant_obj.items():
                for test_name, test_obj in task_obj.items():
    ...
    def bar_graphs(self):
        ...
        for metric, log in metrics:
            ...
            for dataset in self.separate_fio_tests():
                ...
                for variant_name, variant_obj in dataset.items():
                    ...
                    for path, val in deep_dict.iterate(variant_obj):
                        if path[-1] == metric:
                            yvalues.append(val)
                            test_names.append(path[1] + "." +
                                              str(path[2]))  # test_name.thread_level
                            if metric == 'max':
        ...
        for metric, log in metrics:
            ...
            for dataset in self.separate_fio_tests():
                ...
                for variant_name, variant_obj in dataset.items():
                    ...
                    if log:
                        axis.set_yscale('log')
                    for test_name, test_result_array in test_results.items():
                        axis.plot(test_result_array,
                                  label=test_name,
