...
class MongoConfig(object):
    ...
    def __init__(self, root_config, topology, is_mongos=False):
        ...
        self.numactl_prefix = \
            root_config['infrastructure_provisioning'].get('numactl_prefix', '')
...
class NodeTopologyConfig(object):
...
class ShardedTopologyConfig(object):
    """
    ...
    def _create_shard_config(self, topology, delay_graph, root_config):
        ...
        if cluster_type == 'standalone':
            self.node_shards.append(
                NodeTopologyConfig(topology=topology,
                                   root_config=root_config,
                                   delay_graph=delay_graph))
        if cluster_type == 'replset':
            self.repl_shards.append(
                ReplTopologyConfig(topology=topology,
                                   root_config=root_config,
                                   delay_graph=delay_graph))
