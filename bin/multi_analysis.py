...
class MultiEvergreenAnalysis(object):
    ...
    def evergreen_fetch_result_ids(self):
        ...
        for build in self.builds:
            ...
            for build_variant_id in version_obj['builds']:
                ...
                tasks_in_variant = {}
                for task_name, task_obj in build_variant_obj['tasks'].items():
    ...
    def evergreen_fetch_results(self):
        ...
        for build in self.builds:
            build_results = {}
            for variant_name, tasks_in_variant in build['result_ids'].items():
                build_results[variant_name] = {}
                for task_name, task_obj in tasks_in_variant.items():
                    task_id = task_obj['task_id']
                    try:
    ...
    def _ycsb_fix(self):
        ...
        for result in self.results:
            for variant_name, variant in result.items():
                ...
                for task_name, task in variant.items():
    ...
    def aggregate_results(self):
        ...
        for result in self.results:
            for variant_name, variant_result in result.items():
                ...
                for task_name, task_result in variant_result.items():
                    ...
                    for test_result in task_result['data']['results']:
                        ...
                        for thread_level, thread_result in test_result['results'].items():
    ...
    def flat_results(self, labels=None):
        ...
        if not labels:
            labels = {}
        flat_results = []
        for variant_name, variant in self.agg_results.items():
            for task_name, task in variant.items():
                for test_name, test in task.items():
                    for thread_level, thread_results in test.items():
