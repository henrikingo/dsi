...
class TestRunCuratorAndFriends(unittest.TestCase):
    ...
    def test_shell_curator_runner_command_no_bootstrap(self):
        ...
        mock_host.run.assert_has_calls([
            call([
                ...
                'mock-cert', '--key', 'mock-cert', '--ca', 'mock-cert', '--path',
                'cedar_report.json'
            ])
        ])
    ...
    def test_shell_curator_runner_command_with_bootstrap(self):
        ...
        mock_host.run.assert_has_calls([
            call([
                'my-custom-curator-path', 'poplar', 'send', '--service', 'cedar.mongodb.com:7070',
                '--cert', 'mock-cert', '--key', 'mock-cert', '--ca', 'mock-cert', '--path',
                'cedar_report.json'
            ])
        ])
