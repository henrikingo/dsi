...
class HostTestCase(unittest.TestCase):
    ...
    def test_kill_remote_procs(self):
        ...
        calls = [
            call(['pkill', '-SIGKILL', 'mongo'], quiet=True),
    ...
    def test_kill_mongo_procs(self):
        ...
        local.kill_remote_procs.assert_called_once_with('mongo', 'SIGKILL', max_time_ms=30000)
    ...
    def helper_test_checkout_repos(self, source, target, commands, branch=None, verbose=True):
        ...
        with patch('common.host.mkdir_p') as mock_mkdir_p, \
             patch('common.local_host.LocalHost.exec_command') as mock_exec_command:
    ...
    def test_checkout_repos_existing_target(self):
        ...
        target = os.path.join(self.parent_dir, 'stitch-js-sdk')
        command = ['cd', target, '&&', 'git', 'status']
        with patch('common.host.os.path.isdir') as mock_isdir, \
             patch('common.host.mkdir_p') as mock_mkdir_p, \
             patch('common.local_host.LocalHost.exec_command') as mock_exec_command:
