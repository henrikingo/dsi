...
class TestMongoNode(unittest.TestCase):
    ...
    #
    # Below we test each combination of clean_db_dir, clean_logs, journal_dir and is_mongos
    # to be True and False with ttt_t indicating that all are True, ttf_t indicating
    # that all are true except for journal_dir which is False, etc.
    # Note that both journal_dir and is_mongos cannot be True!

    def test_gen_setup_commands_ttt_f(self):
        ...
        self._test_setup_commands(
            ...
            [['rm', '-rf', '/ log/di r//*.log'], ['rm', '-rf', '/ log/di r//core.*'],
             ['rm', '-rf', 'some dir with spaces/diagnostic.data/*'], [
                 'mkdir', '-p', '/ log/di r//'
        ...
        self._test_setup_commands(
            {
                ...
                ['rm', '-rf', 'logdir/mongos.diagnostic.data/*'], ['mkdir', '-p', 'logdir'],
    ...
    def test_gen_setup_commands_tff_t(self):
        """Proper output for _generate_setup_commands with
        clean_db_dir = True
        clean_logs = False
        journal_dir = False
    ...
    def test_gen_setup_commands_tff_f(self):
        """Proper output for _generate_setup_commands with
        clean_db_dir = True
        clean_logs = False
        journal_dir = False
    ...
    def test_gen_setup_commands_ftf_t(self):
        """Proper output for _generate_setup_commands with
        clean_db_dir = False
        clean_logs = True
        journal_dir = False
    ...
    def test_gen_setup_commands_ftf_f(self):
        """Proper output for _generate_setup_commands with
        clean_db_dir = False
        clean_logs = True
        journal_dir = False
    ...
    def test_gen_setup_commands_fff_f(self):
        """Proper output for _generate_setup_commands with
        clean_db_dir = False
        clean_logs = True
        journal_dir = False
    ...
    def test_destroy(self):
        ...
        calls = [mock.call(signal_number='SIGTERM', max_time_ms=1), mock.call()]
        ...
        calls = [mock.call(signal_number='SIGTERM', max_time_ms=2), mock.call()]
        self.mongo_node.host.kill_mongo_procs.assert_has_calls(calls)
        self.mongo_node.host.run.assert_not_called()
